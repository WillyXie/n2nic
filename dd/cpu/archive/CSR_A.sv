...
parameter MSCRATCH  = 12'h340; //for output internal interrupt
...
always_comb begin
    ...
    if(addr == addr_q) csr_rdata = csr_wdata;   // forwarding
        ...
        case(addr)
            ...
            MHARTID  : csr_rdata = 32'd0;   //Read only
        endcase
    end
end
...
// Core Interrupter ------------------------------------
assign int_interrupt_O = (addr == MSCRATCH) ? 1'b1: 1'b0;
always_ff @(posedge clk or posedge rst) begin
    if(rst)
        int_taken <= 1'b0;
    else
...
always_comb begin
    case(state)
        ...
        WAIT : nstate = (int_taken || int_interrupt_I)? IDLE : WAIT; 
    endcase
end
