module control_data(
    ...
    parameter W_HIT  = 4'd9;
    parameter W_MISS = 4'd10;
    parameter W_SYS  = 4'd11;
    parameter REQ_SNP = 4'd12;
    ...
    always_comb begin
        case(state)
            IDLE   : nstate = (core_req)? ((index_arbitor)? REQ_SNP : (core_write)? WRITE : READ) : IDLE;
            REQ_SNP: nstate = (core_write)? WRITE : READ;
            READ   : nstate = (match && valid)? WAIT : R_MISS;
            //R_MISS : nstate = (grant=='CACHE_A) ? R_SYS : R_MISS;
            R_MISS : nstate = (grant) ? R_SYS : R_MISS;
            R_SYS  : nstate = (~L2C_finish_reg)? R_SYS : ((cacheable)? R_DATA : R_WAIT2);
            ...
            //W_HIT : nstate = (grant=='CACHE_A)? W_SYS : W_HIT;
            //W_MISS: nstate = (grant=='CACHE_A)? W_SYS : W_MISS;
            W_HIT : nstate = (grant)? W_SYS : W_HIT;
            W_MISS: nstate = (grant)? W_SYS : W_MISS;
            W_SYS : nstate = (~L2C_finish_reg || ~snp_hit_valid)? W_SYS : WAIT;
            default:nstate = IDLE;
        endcase
    end
    ...
    always_comb begin
        ...
        case(state) 
            ...
            W_HIT: begin
                core_wait = 1'b1;
                D_req     = (snoop) ? 1'b0 :1'b1;
                //status_write = (grant==`CACHE_A)? 1'b1: 1'b0;
                status_write = (snoop)? 1'b0 : ((grant)? 1'b1: 1'b0);
